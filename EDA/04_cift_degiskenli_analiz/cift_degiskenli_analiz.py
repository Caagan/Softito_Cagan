...
def main():
    print("=" * 70)
    print("  04 - CIFT DEGISKENLI ANALIZ")
    print("=" * 70)
    ...
    if len(num_cols) > 1:
        ...
        plt.title("Sayisal Degiskenler Arasi Korelasyon Matrisi")
    ...
    if gender_col and len(num_cols) >= 2:
        ...
        ax.set_xlabel(x_col)
        ax.set_ylabel(y_col)
        ax.set_title(f"{x_col} vs {y_col} ({gender_col} bazinda)")
        ax.legend()
    ...
    if len(cat_cols) >= 2:
        ...
        plt.title(f"{c1} - {c2} Iliskisi (Yuzde)")
        plt.ylabel("Yuzde")
